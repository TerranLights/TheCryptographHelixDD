...
- [ ] **All nation-derived factions — blocked on InnerTepeniaGDD Phase 3 (two-generation neo-culture rework)**
  InnerTepeniaGDD's new "Neo-Races and Neo-Cultures" project (see `reference-neo-races-and-cultures` in memory) synthesizes each Tepenian city's culture fresh from circumstances rather than treating it as a continuation of its founders' real-world nation, and explicitly reserves a Phase 3 for "orbital / Cryptograph Helix era" populations. This is a **two-generation lineage problem**, clarified by the user 2026-07-17: real-world nation → (~250–300 years) → a specific Tepenian city's neo-culture (Phase 1c/2, already complete for all 35 cities) → (~another ~500 years, Falkland Treaty 2564 to Cryptograph Helix's ~3000s–3100s opening) → a *second*, further-composite neo-race/neo-culture in the Mars/orbital setting. Nothing in Cryptograph Helix should still be named after the original real-world nation by the time the novels open — e.g. the "Chinese Mafia" should really descend from Zhongshan (the Chinese-founded, multinational, human Tepenian city — not Kunlun, which is Sinian but 100% robot) and be renamed accordingly (something like "Zhongshanese"), and likewise for the Russian (candidate ancestor: Mirny), Korean, and Brazilian Mafias, and for the Resistance/Hungarian/Altay Nation faction. None of the current `factions/mafias/*/overview.md` or `factions/resistance/overview.md` files trace this lineage yet — this is new ground. User's call: **wait for InnerTepeniaGDD's Phase 3 to actually be developed** before reworking any of this, not to do it independently now. Phase 3 was empty (`.gitkeep` only) as of 2026-07-17 — check back periodically.
