# Completion Matrix

Cross-phase progress at a glance. Update as work is completed.

---

## Series Overview

| File | Status |
|---|---|
| `series-overview/premise.md` | ✅ Complete |
| `series-overview/themes-and-structure.md` | ✅ Complete |
| `series-overview/universe-timeline.md` | ✅ Complete |
| `series-overview/title-candidates.md` | ✅ Created — decisions pending |
| `series-overview/faction-motivations.md` | ✅ Complete |
| `series-overview/grand-percentage-structure.md` | ✅ Complete |
| `series-overview/per-book_breakdown.md` | ✅ Complete — quick per-book seed notes |
| `series-overview/book-spec-sheets.md` | ✅ Complete — full per-book guiding spec, extrapolated from per-book_breakdown.md |

## Factions

| Faction | Overview | 48-Laws | Notes |
|---|---|---|---|
| Earthtech | ✅ | ⬜ Empty | Name TBD |
| Chinese Mafia | ✅ | ✅ | |
| Russian Mafia | ✅ | ✅ | |
| Brazilian Mafia | 🔶 Stub | 🔶 Stub | |
| Korean Mafia | 🔶 Stub | 🔶 Stub | Replaced Serbian — Jeju-do legacy seed |
| Memes | ✅ | 🔶 Partial | Broken list TBD |
| E-Thots (overview) | ✅ | — | |
| Colombian E-Thot | ✅ | ✅ | Name TBD |
| Resistance | ✅ | ⬜ Empty | |

`factions/mafias/z-template/` — a reusable build template (README + overview + 48-laws) capturing the four-stage neo-culture lineage and the No National Stereotypes rule, for use once InnerTepeniaGDD's Phase 3 unblocks the Mafia/Resistance rework.

## Characters

| Character | File | Harmon Arc | 48-Laws | Notes |
|---|---|---|---|---|
| László | ✅ | ✅ | ✅ | Books 2–3 role TBD |
| Colombian e-thot | Via faction | ✅ | ✅ | Name TBD |
| Howie | ✅ | ⬜ | ⬜ | |

## Books

Each book is a folder with `overview.md` + `volume-1/` + `volume-2/`. Each volume has `structure/main story/Save the Cat.md`, `structure/character arcs/`, and `Dev-Road-Map/` (00-Overview + 10-Craft-Reference). Volume 1 always ends at the Midpoint; Volume 2 covers post-Midpoint through Finale.

| Book | Overview | Vol 1 Beat Sheet | Vol 2 Beat Sheet | Notes |
|---|---|---|---|---|
| 1 — Terranocracy Anointed | ✅ | 🔶 Partial — Act 1 beats populated | 🔶 Partial — Doorway to Act #3 populated | |
| 2 — The Autonomy Allele | ✅ | 🔶 Partial — IoT assassination + piano-key | ⬜ Stub | László introduced ~2.10 |
| 3 — Neurotropic Dreamtime | ✅ | 🔶 Partial — Russian Mafia in Road of Trials | 🔶 Partial — Meeting with the Goddess | |
| 4 — The Architect Awakes | ✅ | 🔶 Partial — Road of Trials + Midpoint populated | 🔶 Partial — Atonement with the Father | László arc begins 4.1; memes online ~4.7 |
| 5 — Eclipse of a Ganymede Sunrise | ✅ | ⬜ Stub | 🔶 Partial — Dark Night + Final Battle populated | |
| 6 — Storm over Callisto | ✅ | ⬜ Stub | 🔶 Partial — Bad Guys Close In + Dark Night | |
| 7 — Embers of a Frostbitten Star | ✅ | ⬜ Stub | 🔶 Partial — Q-Factor + Final Battle + Closing Image | ⚠️ Needs full revision — standalone-universe content |

## Plot

| Document | Status | Notes |
|---|---|---|
| `plot/series-map.md` | ✅ Complete | |
| `plot/monomyth.md` | 🔶 Partial | Iteration 1 done; 2–7 stubbed |
| `plot/parallel-plots/mafia.md` | ✅ Complete | Full 3-circle Harmon arc |
| `plot/parallel-plots/meme-nation.md` | ✅ Complete | |
| `plot/parallel-plots/colombian-e-thot.md` | ✅ Complete | |
| `plot/parallel-plots/laszlo.md` | ✅ Complete | |

## Societies

| Society | Status |
|---|---|
| Hungarians | 🔶 Placeholder |
| Finns | 🔶 Placeholder |
| Chinese | 🔶 Placeholder |
| Colombians | 🔶 Placeholder |
| Greeks | 🔶 Placeholder |
| Germans | 🔶 Placeholder |

## World

| File | Status |
|---|---|
| `world/technology.md` | ✅ Complete |
| `world/locations.md` | ✅ Complete — will expand |
| `world/society.md` | ✅ Complete — will expand |
| `world/outer-tepenia.md` | ✅ Complete |
| `world/navigation.md` | ✅ Complete — 3D heliocentric coordinate system; naming/epoch decisions pending |

## Philosophy

| File | Status |
|---|---|
| `philosophy/insights.md` | ✅ Complete — will expand |
| `philosophy/possibilities.md` | ✅ Complete — will expand |
| `philosophy/48-laws-reference.md` | ✅ Complete |

## Reference

| File | Status |
|---|---|
| `reference/story-structure-definitions.md` | ✅ Complete — fused Snyder/Bell/Truby/Campbell definitions |
| `reference/future-ideas.md` | ✅ Active |
| `reference/unsorted-notes.md` | ✅ Active |
| `reference/continuity-guidelines.md` | ✅ Complete |
| `reference/ebooks/` | ✅ Collecting — gitignored |

---

**Legend:** ✅ Complete · 🔶 Partial or placeholder · ⬜ Empty / not started
